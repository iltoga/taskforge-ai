...
export class ToolOrchestrator {
  ...
  async orchestrate(
    ...
    const { maxSteps = 10, maxToolCalls = 5, developmentMode = false } = config;

    // Handle file context integration for supported models
    if (processedFiles.length > 0) {
      console.log(
        `📄 Orchestrator received ${processedFiles.length} processed files for context, routing to file processing:`,
        processedFiles
      );
      ...
      try {
        ...
        if (processedFiles.length > 0) {
          console.log(
            `📸 Processing ${processedFiles.length} processed files with orchestrator`
          );
        ...
        this.logProgress(
          ...
          } - continuing with regular orchestration`
        );
      }
    }
    ...
    try {
      this.logProgress(
        `🎯 Starting orchestration for query: "${userMessage.substring(0, 50)}${
          userMessage.length > 50 ? "..." : ""
        }"`
      );
      this.logProgress(
        `⚙️ Using model: ${model} | Max steps: ${maxSteps} | Max tool calls: ${maxToolCalls}`
      );

      // Build internal conversation context that accumulates as we progress
      const internalConversation: Array<{
        role: "user" | "assistant";
        content: string;
      ...
      const analysisStep = await this.performAnalysis(
        userMessage,
        chatHistory,
        toolRegistry,
        model,
      ...
      internalConversation.push({
        role: "assistant",
        content: `Analysis: ${analysisStep.content}`,
      });
      ...
      while (
        ...
        this.logProgress(
          `🤔 Iteration ${steps.length}: Deciding on tool usage (${toolCallCount}/${maxToolCalls} tools used)...`
        );
        ...
        internalConversation.push({
          role: "assistant",
          content: `Tool Planning: ${toolDecisionStep.content}`,
        });

        // Parse tool decisions and execute tools
        let toolsToCall = this.parseToolDecisions(toolDecisionStep.content);

        // --- Smart retry & fallback when LLM selects no tools -----------------
        if (toolsToCall.length === 0) {
          this.logProgress(
            `⚠️ LLM returned no CALL_TOOLS. Retrying once with explicit instruction...`
          );

          // Re‑prompt the LLM with a stricter note that a tool *must* be used
          const retryContext =
            currentContext +
            "\n\nSYSTEM_NOTE: Your previous response contained NO CALL_TOOLS. You MUST select and call at least one tool. If uncertain, choose the most relevant search/query tool.";
          const retryDecisionStep = await this.decideToolUsage(
            retryContext,
            toolRegistry,
            toolCalls,
            steps,
          ...
          steps.push(retryDecisionStep);
          internalConversation.push({
            role: "assistant",
            content: `Tool Planning (retry): ${retryDecisionStep.content}`,
          });

          toolsToCall = this.parseToolDecisions(retryDecisionStep.content);
        }

        // If the retry still yields no tools, fall back to a safe default
        if (toolsToCall.length === 0) {
          this.logProgress(
            `❌ Retry still produced no tools. Falling back to default search tool to satisfy "at least one tool" rule.`
          );

          if (this.isCalendarQuery(userMessage)) {
            // Calendar‑centric query → default to getEvents
            toolsToCall = [
              {
                name: "getEvents",
                parameters: {},
            ...
            toolRegistry
              .getAvailableTools()
              .some((t) => t.name === "vectorFileSearch")
          ) {
            // General knowledge query → default to vectorFileSearch
            toolsToCall = [
              {
          ...
          this.logProgress(
            `🔧 Auto‑fallback tool planned: ${toolsToCall
              .map((t) => t.name)
              .join(", ")}`
          );
        }
        ...
        this.logProgress(
          `🔧 Planning to execute ${toolsToCall.length} tools: ${toolsToCall
            .map((t) => t.name)
            .join(", ")}`
        );
        ...
        for (const toolCall of toolsToCall) {
          ...
          if (
            toolCall.name === "vectorFileSearch" &&
            toolCall.parameters &&
            !toolCall.parameters.vectorStoreIds
          ) {
            try {
              const fs = await import("fs");
              const path = await import("path");
              ...
              this.logProgress(
                `⚠️ Could not load vectorStoreIds from config: ${
                  error instanceof Error ? error.message : "Unknown error"
                }`
              );
            }
          ...
          this.logProgress(
            `🛠️ Tool "${toolCall.name}" parameters: ${JSON.stringify(
              toolCall.parameters,
              null,
              2
            )}`
          ...
          this.logProgress(
            `📥 Tool "${toolCall.name}" result: ${JSON.stringify(
              result,
              null,
              2
            )}`
          ...
          if (toolCall.name === "vectorFileSearch") {
            const vectorStoreIds =
              toolCall.parameters && toolCall.parameters.vectorStoreIds
                ? JSON.stringify(toolCall.parameters.vectorStoreIds)
                : "N/A";
            const returnedData =
              result && result.data
          ...
          if (
            toolCall.name === "searchEvents" ||
            toolCall.name === "getEvents"
          ) {
            if (result.success) {
              const eventCount = Array.isArray(result.data)
                ? result.data.length
                : 0;
              this.logProgress(
                `📅 Calendar tool ${
                  toolCall.name
                } succeeded: Found ${eventCount} events (${
                  endTime - startTime
                }ms)`
              );
              if (eventCount > 0 && Array.isArray(result.data)) {
                const eventTitles = result.data
                  .slice(0, 3)
                  .map((e: unknown) => {
              ...
              this.logProgress(
                `❌ Calendar tool ${toolCall.name} failed: ${
            ...
            if (result.success) {
              const eventTitle =
                ...
                "summary" in result.data
                  ...
                  : "New event";
              this.logProgress(
                `✅ Event created successfully: ${eventTitle} (${
                  endTime - startTime
                }ms)`
              );
            } else {
          ...
          const resultSummary = execution.result.success
            ? `Tool ${toolCall.name} succeeded: ${JSON.stringify(
        ...
        internalConversation.push({
          role: "assistant",
          content: `Evaluation: ${evaluationStep.content}`,
        });
        ...
        needsMoreInformation = this.needsMoreInformation(
          evaluationStep.content
        );
        this.logProgress(
          `📊 Evaluation result: ${
            needsMoreInformation
        ...
        if (!needsMoreInformation && this.isCalendarQuery(userMessage)) {
          const hasSuccessfulCalendarTools = toolCalls.some(
            (call) =>
              (call.tool === "searchEvents" || call.tool === "getEvents") &&
              call.result.success
          );

          const hasAttemptedCalendarTools = toolCalls.some(
            (call) => call.tool === "searchEvents" || call.tool === "getEvents"
          );

          const calendarToolCalls = toolCalls.filter(
            (call) => call.tool === "searchEvents" || call.tool === "getEvents"
          );

          // Force continuation if no calendar tools were attempted at all, but only if we haven't hit limits
          if (
            !hasAttemptedCalendarTools &&
            toolCallCount < maxToolCalls &&
            steps.length < maxSteps
          ) {
            needsMoreInformation = true;
            this.logProgress(
              `🔄 Forcing tool retry for calendar query - no calendar tools attempted yet (${toolCallCount}/${maxToolCalls} tool calls used)`
            );
            ...
            const failedAttempts = calendarToolCalls.filter(
              (call) => !call.result.success
            ).length;
            this.logProgress(
              `⚠️ Calendar tools attempted but failed - proceeding with error explanation (${failedAttempts} failed attempts)`
            );
            ...
            calendarToolCalls.forEach((call) => {
              if (!call.result.success) {
                this.logProgress(
                  `   └─ ${call.tool}: ${
                    call.result.error || call.result.message || "Unknown error"
                  }`
                );
              }
            });
            ...
            this.logProgress(
              `⚠️ Calendar query completed without attempting calendar tools due to limits reached (${toolCallCount}/${maxToolCalls} tool calls, ${steps.length}/${maxSteps} steps)`
            );
          } else {
            // Success case - we have successful calendar tools
            const successfulAttempts = calendarToolCalls.filter(
              (call) => call.result.success
            ).length;
            this.logProgress(
              `✅ Calendar tools executed successfully (${successfulAttempts} successful attempts)`
            );
          }
        }
      }
      ...
      this.logProgress(
        `🧠 Synthesizing response from ${toolCalls.length} tool executions...`
      );
      ...
      this.logProgress(
        `📊 Tool execution summary: ${successfulTools} successful, ${failedTools} failed`
      );

      // Use synthesis tool instead of internal method
      ...
      let synthesisStep: OrchestrationStep;
      if (synthesisResult.success && synthesisResult.data) {
        ...
        // Fallback if synthesis tool fails
        synthesisStep = {
          id: `step_${currentStepId++}`,
          type: "synthesis",
          timestamp: Date.now(),
          content:
            "I encountered an error while synthesizing the response. Please try again.",
          reasoning: "Synthesis tool execution failed",
        };
      }
      ...
      while (synthesisIterations < maxSynthesisIterations) {
        this.logProgress(
          `🔍 Validating response format (iteration ${synthesisIterations}/${maxSynthesisIterations})...`
        );
        ...
        this.logProgress(
          `🔄 Refining response based on validation feedback...`
        );
      ...
      if (synthesisIterations >= maxSynthesisIterations) {
        this.logProgress(
          `⚠️ Response format validation reached maximum iterations (${maxSynthesisIterations})`
        );
      }

      const totalDuration = Date.now() - (steps[0]?.timestamp || Date.now());
      this.logProgress(
        `🎉 Orchestration completed successfully in ${totalDuration}ms (${steps.length} steps, ${toolCalls.length} tool calls)`
      );
      ...
      return {
        ...
        steps: developmentMode
          ? steps
          : steps.filter((step) => step.type === "synthesis"),
        toolCalls,
      };
    } catch (error) {
      this.logProgress(
  ...
  private generateDecisionRules(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("calendar")) {
      ...
      const hasCreateEvent = calendarTools.some(
        (t) => t.name === "createEvent"
      );
      const hasUpdateEvent = calendarTools.some(
        (t) => t.name === "updateEvent"
      );
      const hasDeleteEvent = calendarTools.some(
        (t) => t.name === "deleteEvent"
      );
      const hasSearchEvents = calendarTools.some(
        (t) => t.name === "searchEvents"
      );
      ...
      if (hasSearchEvents || hasGetEvents) {
        rules += `${ruleNumber}. **FOR PROJECT/WORK QUERIES**: **ALWAYS** search calendar first using ${
          hasSearchEvents ? "searchEvents" : ""
        }${hasSearchEvents && hasGetEvents ? " or " : ""}${
          hasGetEvents ? "getEvents" : ""
        }\n`;
    ...
    if (availableCategories.includes("passport")) {
      ...
      if (passportTools.length > 0) {
        ...
        rules +=
          "   - **IMAGE UPLOADS**: When users upload passport images, analyze the image and extract passport information\n";
        rules +=
          '   - **PASSPORT CREATION**: "store", "save", "add", "create", "register" passport data → Use createPassport tool\n';
        rules +=
          '   - **PASSPORT LOOKUP**: "find", "search", "get", "show", "display", "list" passport data → Use getPassports tool\n';
        rules +=
          '   - **PASSPORT UPDATE**: "update", "modify", "change", "edit" passport data → Use updatePassport tool\n';
        rules +=
          '   - **PASSPORT DELETION**: "delete", "remove", "clear" passport data → Use deletePassport tool\n';
        rules +=
          '   - **SCHEMA SETUP**: "setup", "initialize", "create table" → Use setupPassportSchema tool\n';
        rules +=
          "   - **CRITICAL**: When processing passport images, extract all visible fields and create comprehensive passport records\n";
        rules +=
          "   - Always validate required fields: passport_number, surname, given_names, nationality, dates\n\n";
        ruleNumber++;
      }
    }
    ...
    rules += `${
      ruleNumber + 1
    }. **Enough info?** – Say so and explain briefly.\n\n`;
  ...
  private generatePriorityOrder(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("calendar")) {
      const calendarTools = toolRegistry.getToolsByCategory("calendar");
      const eventTools = calendarTools.filter((t) =>
        ["createEvent", "updateEvent", "deleteEvent"].includes(t.name)
      );
      const searchTools = calendarTools.filter((t) =>
        ["searchEvents", "getEvents"].includes(t.name)
      );

      if (eventTools.length > 0) {
        priority += `- Event creation/modification → ${eventTools
          .map((t) => t.name)
          .join(", ")}\n`;
      }
      if (searchTools.length > 0) {
        priority += `- Calendar queries → ${searchTools
          .map((t) => t.name)
          .join(", ")}\n`;
      }
    }
  ...
  private generateAnalysisInstructions(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("passport")) {
      instructions +=
        "- Use passport tools for passport and document processing, including extracting data from passport images and managing passport records.\n";
      instructions +=
        "- When creating a new passport record, all fields except `surname` and `given_names` must be translated to English before uploading to the database.\n";
    }
    ...
    if (
      availableCategories.includes("web") &&
      !allTools.some((t) => t.name === "vectorFileSearch")
    ) {
      instructions +=
        "- Use web tools for online information gathering when needed.\n";
    }
  ...
  private generateContextInstructions(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("passport")) {
      context +=
        "**PASSPORT PROCESSING CONTEXT**: This application can process passport documents and images.\nAny mention of:\n";
      context += "- Uploaded passport images or document photos\n";
      context +=
        '- "Analyze this passport", "extract passport data", "read passport information"\n';
      context +=
        '- "Save passport details", "store passport data", "create passport record"\n';
      context +=
        '- "Find passport", "search passport", "get passport information"\n';
      context +=
        "- Passport numbers, expiry dates, personal information from documents\n";
      context +=
        "- Document processing, data extraction, OCR of identity documents\n\n";
      context +=
        "Should **ALWAYS** be interpreted as **PASSPORT PROCESSING REQUESTS** that require using passport tools to analyze images and manage passport data.\n\n";
    }
  ...
  private generateAnalysisExamples(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("passport")) {
      ...
      examples +=
        '- "create passport record from document" → Use createPassport with extracted passport data\n';
      examples +=
        '- "save passport information" → Use createPassport with all required fields\n';
      examples +=
        '- "find passport P12345" → Use getPassports with passport_number: "P12345"\n';
      examples +=
        '- "search for John Doe passport" → Use getPassports with surname: "DOE" and given_names: "JOHN"\n';
      examples +=
        '- "update passport details" → Use updatePassport with id and updated fields\n';
      examples += '- "delete passport record" → Use deletePassport with id\n';
      examples +=
        '- "list all passports" → Use listPassports without filters\n';
      examples +=
        '- "analyze passport image" → Use createPassport with extracted data from the image\n';
      examples +=
        '- "extract passport data from uploaded image" → Use createPassport with extracted data from the image\n';

      examples += "**CRITICAL RULES FOR PASSPORT PROCESSING**:\n";
      examples +=
        "1. ALWAYS include ALL required fields when using createPassport: passport_number, surname, given_names, nationality, date_of_birth, sex, place_of_birth, date_of_issue, date_of_expiry, issuing_authority, holder_signature_present (boolean), type (string)\n";
      examples +=
        "2. For holder_signature_present: use true if signature is mentioned as present, false otherwise\n";
      examples +=
        '3. For type: use "passport" or specific passport type mentioned in the document\n';
      examples += "4. Dates must be in ISO format (YYYY-MM-DD)\n";
      examples +=
        "5. When processing document text, extract all available information and use reasonable defaults for missing required fields\n\n";
    }

    // Combined usage example only if both calendar and vector search are available
    if (
      availableCategories.includes("calendar") &&
      allTools.some((t) => t.name === "vectorFileSearch")
    ) {
      examples += "**IMPORTANT**:\n";
      examples +=
  ...
  private generateToolExamples(toolRegistry: ToolRegistry): string {
    ...
    if (availableCategories.includes("passport")) {
      examples += "**MANDATORY EXAMPLES FOR PASSPORT QUERIES**:\n";
      examples +=
        '- "create passport record" → Use createPassport with all required fields (passport_number, surname, given_names, nationality, date_of_birth, sex, place_of_birth, date_of_issue, date_of_expiry, issuing_authority, holder_signature_present, type)\n';
      examples +=
        '- "extract passport data from document (or image or PDF)" → Use createPassport with passport information from uploaded document\n';
      examples +=
        '- "find passport P12345" → Use getPassports with passport_number: "P12345"\n';
      examples +=
        '- "search for John Doe passport" → Use getPassports with surname: "DOE" and given_names: "JOHN"\n';
      ...
      examples += '- "delete passport record" → Use deletePassport with id\n';
      examples +=
        '- "list all passports" → Use listPassports and summarize the results for names, passport numbers, expiration dates grouped by nationality\n\n';
      ...
      examples += "4. Dates must be in ISO format (YYYY-MM-DD)\n";
      examples +=
        "5. When processing uploaded documents, extract all available information and use reasonable defaults for missing required fields\n\n";
    }
  ...
  private getToolParameterInfo(toolName: string): string {
    ...
    switch (toolName) {
      ...
      case "vectorFileSearch":
        return `{ query: string (required), maxResults?: number, vectorStoreIds: [${this.vectorStoreIds
          .map((id) => `"${id}"`)
          .join(", ")}] (required) }`;
      ...
      case "synthesizeFinalAnswer":
        return "{ userMessage: string (required), chatHistory: array (required), toolCalls: array (required), previousSteps: array (required), model: string (required), stepId: number (required), aiConfig?: object (optional) }";
  ...
  private async performAnalysis(
    ...
    // build file context information using the processed files
    const fileContextInfoMap = processedFiles.map((file) => ({
      fileName: file.fileName,
      fileSize: file.fileSize,
      isImage: file.isImage,
    }));
    const fileContextInfo = fileContextInfoMap.length
      ? `\n\n**FILE CONTEXT INFORMATION**:\n${fileContextInfoMap
          .map(
            (file) =>
              `- ${file.fileName} (${file.fileSize} bytes, ${
                file.isImage ? "image" : "text or other file type"
              })`
          )
          .join("\n")}`
      : "";
...
You are an **AI ASSISTANT** with intelligent tool orchestration capabilities. Your primary purpose is to use the right tools to assist users with their requests.
Analyze the user's request and determine the best tool to use for the next steps to answer their query, given the available tools and context.
...
`;
      ...
      processedFiles && processedFiles.length > 0
        ? await Promise.all(
            processedFiles
              ...
              .map(async (file) => ({
                imageData: await uploadedFileToImageDataUrl(
                  file.fileName,
                  file.fileType
                ),
    ...
    return {
      ...
      reasoning:
        "Comprehensive analysis of user request and strategic tool planning",
    };
  }
  ...
  private async decideToolUsage(
    ...
    const fileContextMap = await Promise.all(
      processedFiles.map(async (file) => {
        let fileContent: Buffer | undefined = undefined;
        if (file.fileName) {
          try {
            const path = await import("path");
            const filePath = path.join(FILE_UPLOAD_DIR, file.fileName);
    ...
    const fileContextInfo = fileContextMap.length
      ? `\n\n**FILE CONTEXT INFORMATION**:\n${fileContextMap
          .map((file) => {
            const contentPreview =
              file.fileContent && file.fileContent.length > 100
                ? file.fileContent.toString().substring(0, 100) + "..."
                : file.fileContent?.toString() || "No content";
            return `- ${file.fileName} (${file.fileSize} bytes): ${contentPreview}`;
          })
          .join("\n")}`
      : "";

    const previousCallsContext =
      previousToolCalls.length > 0
        ? `\nPrevious tool calls and results:\n${previousToolCalls
            .map((call) => {
              const result =
                typeof call.result.data === "object"
                  ? JSON.stringify(call.result.data, null, 2).substring(
                      0,
                      500
                    ) + "..."
                  : call.result.data;
...
${
  internalConversation && internalConversation.length > 1
    ? this.formatInternalConversation(internalConversation)
    : ""
}
...
`;
    ...
    const images =
      processedFiles && processedFiles.length > 0
        ? await Promise.all(
            processedFiles
              .filter((file) => file.fileName && file.isImage)
              .map(async (file) => ({
                imageData: await uploadedFileToImageDataUrl(
                  file.fileName,
                  file.fileType
    ...
    const toolResults = toolCalls
      .map((call) => {
        ...
        const data = call.result.data
          ? typeof call.result.data === "object"
            ? JSON.stringify(call.result.data, null, 2).substring(0, 800) +
              (JSON.stringify(call.result.data).length > 800 ? "..." : "")
            : call.result.data
          : "No data";
...
`;
    ...
    return {
      ...
      content: response?.text || "No response text available",
      reasoning:
        "Comprehensive evaluation of information completeness and next steps",
    };
  }
...
`;
  ...
  private parseToolDecisions(
    content: string
  ): Array<{ name: string; parameters: Record<string, unknown> }> {
    this.logProgress(
      `🔍 Parsing tool decisions from content: ${content.substring(0, 500)}...`
    );

    try {
      // Look for CALL_TOOLS: section with JSON
      const callToolsMatch = content.match(
        /```json[\s\S]*?CALL_TOOLS:\s*(\[[\s\S]*?\])\s*```/
      );
      if (callToolsMatch) {
        this.logProgress(
          `📋 Found CALL_TOOLS in json block: ${callToolsMatch[1]}`
        );
        ...
        if (Array.isArray(tools)) {
          ...
          this.logProgress(
            `✅ Successfully parsed ${parsedTools.length} tools from json block`
          );
          return parsedTools;
        }
      }
      ...
      if (fallbackMatch) {
        this.logProgress(
          `📋 Found CALL_TOOLS without block: ${fallbackMatch[1]}`
        );
        ...
        if (Array.isArray(tools)) {
          ...
          this.logProgress(
            `✅ Successfully parsed ${parsedTools.length} tools from fallback`
          );
          return parsedTools;
        }
      }
      ...
      const executeMatch = content.match(
        /EXECUTE:\s*(\w+)[\s\S]*?PARAMETERS:\s*(\{[\s\S]*?\})/
      );
      if (executeMatch) {
        ...
        try {
          ...
          this.logProgress(
            `❌ Error parsing EXECUTE parameters: ${parseError}`
          );
        }
      }
      ...
      if (jsonArrayMatch) {
        try {
          ...
          if (Array.isArray(tools) && tools.length > 0 && tools[0].name) {
            ...
            this.logProgress(
              `✅ Successfully parsed ${parsedTools.length} tools from JSON array fallback`
            );
            return parsedTools;
          }
        } catch {
  ...
  private buildUpdatedContext(
    ...
    const toolResults = toolCalls
      .map((call) => `${call.tool}: ${JSON.stringify(call.result, null, 2)}`)
      .join("\n");
...
`;
  ...
  private formatInternalConversation(
    ...
    const formatted = internalConversation
      .slice(1)
      .map((msg, index) => {
        const step = index + 1;
   ...
   */
  private async forcePassportToolExecution(
    userMessage: string,
    chatHistory: ChatHistory,
    toolRegistry: ToolRegistry,
    model: ModelType,
...
CHAT HISTORY (use it to understand context and retrieve relevant data):
${chatHistory}

AVAILABLE TOOLS:
${categorizedToolsList}

note: in this case, you MUST use the 'customer-passport' tool to handle passport data.
...
`;
    ...
    // Build images array if we have processed files
    // Use processedFiles to build images array if any are images with fileContent
  ...
  private async executeToolCall(
    ...
    if (result.success) {
      this.logProgress(
        `✅ Tool ${toolCall.name} completed successfully (${execution.duration}ms)`
      );
    } else {
      this.logProgress(
        `❌ Tool ${toolCall.name} failed: ${
   ...
   */
  private buildContextFromToolCalls(
    userMessage: string,
    toolCalls: ToolExecution[]
  ): string {
    const toolResults = toolCalls
