...
export function registerEmailTools(
  ...
  const TOOL_CATEGORY = "email";

  registry.registerTool(
    {
      name: "sendEmail",
      description:
        "Send an email message to recipients. Use this to compose and send emails.",
      parameters: z.object({
        emailData: z
          .object({
  ...
  registry.registerTool(
    {
      name: "searchEmails",
      description:
        "Search for emails with various filters. Use this to find specific emails.",
      parameters: z.object({
        filters: z
          .object({
            ...
            isRead: z
              .boolean()
              .optional()
              .describe("Filter by read/unread status"),
            dateRange: z
              .object({
                start: z
                  .string()
                  .optional()
                  .describe("Start date for search range"),
                end: z
                  .string()
                  .optional()
                  .describe("End date for search range"),
              })
              .optional()
  ...
  registry.registerTool(
    {
      name: "replyToEmail",
      description:
        "Reply to an existing email. Use this to respond to received emails.",
      parameters: z.object({
    ...
    async (params: Record<string, unknown>) => {
      const p = params as {
        emailId: string;
        replyData: { body: string; replyAll?: boolean };
      };
