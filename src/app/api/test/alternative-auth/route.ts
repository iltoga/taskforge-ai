...
export async function GET(request: Request) {
  try {
    ...
    if (serviceAccountAvailable) {
      try {
        ...
        if (serviceAuth) {
          ...
          console.log(`✅ Service account test successful, auth type: ${authType}`);
    ...
    if (session?.accessToken && !forceServiceAccount) {
      ...
      authResults.tests.userOAuth = {
        success: false,
        error: forceServiceAccount ? 'Skipped (forced service account)' : 'User not authenticated'
      };
    }
    ...
    try {
      ...
      const fallbackService = await EnhancedCalendarService.createWithFallback(primaryAuth, forceServiceAccount);
      const authType = fallbackService.getAuthType();
      const canFallback = fallbackService.canFallbackToServiceAccount();

      console.log(`✅ Fallback test successful, auth type: ${authType}, can fallback: ${canFallback}`);
    ...
    const successfulTests = allTests.filter((test: TestResult) => test.success).length;
    ...
    console.log(`🎯 Alternative authentication test summary: ${successfulTests}/${totalTests} tests passed`);
