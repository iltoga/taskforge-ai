...
describe("AIService", () => {
  ...
  describe("processMessage", () => {
    it("should parse a create event request correctly", async () => {
      // Arrange
      const userMessage =
        "Create a meeting with John tomorrow at 2 PM about project review";
    ...
    it("should handle daily work report creation", async () => {
      // Arrange
      const userMessage =
        "Create daily report for TechCorp: worked on API integration, fixed bugs in payment system";
      const mockContent = JSON.stringify({
        type: "create",
        event: {
    ...
    it("should handle update daily report request", async () => {
      // Arrange
      const userMessage =
        "Update today's daily report: added code review session with team";
...
This week focused primarily on API development and code quality improvements. Successfully completed bug fixes and enhanced documentation.`,
  ...
  describe("translateToEnglish", () => {
    it("should translate non-English text to English", async () => {
      ...
      // Assert
      expect(result).toBe(
        "Meeting with the development team tomorrow at 3:00 PM"
      );
    });
