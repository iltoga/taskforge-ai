...
describe("Vector Store IDs Integration Test", () => {
  it("should pass vectorStoreIds to vectorFileSearch tool", async () => {
    ...
    try {
      ...
      const vectorSearchLogs = logs.filter(
        (log) =>
          log.includes("vectorFileSearch") || log.includes("vectorStoreIds")
      );
      console.log("Vector search related logs:", vectorSearchLogs);

      // Look for logs that show vectorStoreIds
      const vectorStoreIdLog = logs.find(
        (log) => log.includes("vectorStoreIds") && !log.includes("N/A")
      );
      expect(vectorStoreIdLog).toBeDefined();

      if (vectorStoreIdLog) {
        console.log("Found vectorStoreIds in log:", vectorStoreIdLog);
        expect(vectorStoreIdLog).toContain(
          "vs_6856abe00a4081918be9af94278a7f2c"
        );
      }
    } catch (error) {
      console.log(
        "Expected error (likely no API key):",
