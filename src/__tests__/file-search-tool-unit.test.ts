...
describe("FileSearchTool - Unit Tests", () => {
  ...
  describe("PDF Processing Logic", () => {
    it("should handle empty file IDs list", async () => {
      ...
      await expect(
        fileSearchTool.initialize([], "test instructions", "gpt-5-mini")
    ...
    it("should format queries correctly for different scenarios", () => {
      ...
      const isContentQuery = (q: string) =>
        ...
        (q.toLowerCase().includes("see") ||
          ...
          q.toLowerCase().includes("content"));
  ...
  describe("File Search Fallback Logic", () => {
    it("should detect failed file processing scenarios", () => {
      ...
      const failedFiles = mockVectorStoreFiles.filter(
    ...
    it("should validate content analysis logic", () => {
      const passportResponse =
        "This passport shows nationality, date of birth, and passport number";
      const tetherResponse =
        "This document shows tether governance structure and business model";
