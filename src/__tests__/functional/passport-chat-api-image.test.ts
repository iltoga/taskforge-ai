...
describe('Full chat API flow with image upload', () => {
  ...
  beforeAll(async () => {
    ...
    console.log(`✅ Test image encoded to base64 (${imageBase64.length} chars - truncated for testing)`);
  }, 30000);
  ...
  it('processes passport image through full chat API', async () => {
    ...
    if (result.toolCalls && result.toolCalls.length > 0) {
      console.log('🔧 Tool calls made:', result.toolCalls.map((tc: any) => tc.tool));

      // Look for passport creation tool calls
      const passportToolCalls = result.toolCalls.filter((tc: any) =>
        tc.tool === 'createPassport' || tc.tool === 'setupPassportSchema'
      );

      if (passportToolCalls.length > 0) {
        console.log('✅ Passport tools were called - image was processed successfully');
        expect(passportToolCalls.length).toBeGreaterThan(0);
      } else {
        console.warn('⚠️ No passport tools were called - image processing may have failed');
      }
    } else {
      console.warn('⚠️ No tool calls were made - this suggests image processing failed');
    }
  });
});
