...
describe("KITAS Query Functional Test", () => {
  ...
  it("should use vectorFileSearch for KITAS modification question", async () => {
    const query =
      "My company director has a KITAS but was told by immigration he cannot perform his role as a DJ, even though it's for his own company's events. Can his permit be modified?";
    ...
    result.steps?.forEach((step) => {
      console.log(
        `[${step.type.toUpperCase()}] ${step.content.substring(0, 150)}...`
      );
    });
    ...
    result.toolCalls?.forEach((call) => {
      console.log(
        `- ${call.tool}: ${call.result.success ? "✅" : "❌"} ${
          call.result.data || call.result.error
        }`
      );
    });

    // Assert vectorFileSearch was used
    const usedVectorSearch = result.toolCalls?.some(
      (call) => call.tool === "vectorFileSearch" && call.result.success
    );
    ...
    if (!usedVectorSearch) {
      ...
      console.error(
        "1. Vector store IDs in config/vector-search.json are correct"
      );
      console.error(
        "2. The knowledge base contains relevant KITAS information"
      );
