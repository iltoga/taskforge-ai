...
describe('Chat API - Nespola Filtering Test', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('should correctly filter nespola events from march to june 2025', async () => {
    ...
    // Mock AI service to return correct action for nespola query
    ...
    const mockAITranslate = jest.fn().mockResolvedValue('list all events relative to nespola from march to june 2025');
    ...
    console.log('📋 Event titles:', filteredEvents.map((e: any) => e.summary));
    ...
    console.log('✅ Test passed: Only events containing "nespola" were returned');
  });
  ...
  test('should handle case-insensitive filtering', async () => {
    ...
    MockedAIService.mockImplementation(() => ({
      ...
      translateToEnglish: jest.fn().mockResolvedValue('list events for nespola'),
    } as any));
