...
describe("Chat Flow - Orchestrator Triggering (Mocked Session)", () => {
  beforeAll(() => {
    ...
    if (!googleClientId) {
      throw new Error(
        "GOOGLE_CLIENT_ID not found in environment variables. Make sure you have a .env file with GOOGLE_CLIENT_ID set."
      );
    }

    if (!googleClientSecret) {
      throw new Error(
        "GOOGLE_CLIENT_SECRET not found in environment variables. Make sure you have a .env file with GOOGLE_CLIENT_SECRET set."
      );
    }
  ...
  beforeEach(() => {
    ...
    console.log(
      "🔐 Mocked authenticated session with FAKE tokens (cannot access real calendar data)"
    );
  });
  ...
  it("should verify the orchestrator is triggered for calendar queries (NOTE: fake tokens cannot access real calendar)", async () => {
    ...
    try {
      ...
      if (fullResponse.toLowerCase().includes("training data")) {
        ...
        console.warn(
          "⚠️  UNCLEAR: Response doesn't mention techcorpor training data"
        );
      }
      ...
      if (
        !toolCallsDetected &&
        fullResponse.toLowerCase().includes("training data")
      ) {
        throw new Error(
          "Orchestrator was not triggered - still getting training data response despite authentication"
        );
      }
