  ...
  test("complete agentic workflow: query → analysis → tool execution → simplified events → summary", async () => {
    ...
    mockGenerateText
      .mockResolvedValueOnce({
        text: "Analysis: User wants Techcorpevents from March-June 2025",
      })
      .mockResolvedValueOnce({
        text: `\`\`\`json
CALL_TOOLS:
...
\`\`\``,
      ...
      .mockResolvedValueOnce({
        text: "COMPLETE: Found 2 Techcorpevents in the timeframe",
      })
      .mockResolvedValueOnce({
        text: `# TechcorpEvents Summary

Found 2 Nespola-related events:

**1. TechcorpTeam Meeting** - April 15, 2025 (2:00-3:00 PM)
- Location: Conference Room B
- 2 attendees
- Weekly sync meeting
...
Both events show active Techcorpproject work during Q2 2025.`,
      })
    ...
    console.log(
      `  ✅ Data format: SimplifiedEvent[] (not verbose CalendarEvent[])`
    );
...
\`\`\``,
      ...
      .mockResolvedValueOnce({
        text: "COMPLETE: Tool failed, can explain error",
      })
      .mockResolvedValueOnce({
        text: "Unable to search calendar events due to API timeout.",
      })
      // Add format validation response
      .mockResolvedValueOnce({
        text: "FORMAT_ACCEPTABLE: The response properly addresses the user's request with appropriate format and content structure.",
    ...
    console.log(
      "🛡️ Error handling verified: Agentic mode gracefully handles tool failures"
    );
  });
});
