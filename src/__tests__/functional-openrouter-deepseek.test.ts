...
describe("Functional Test: OpenRouter Gemini Integration", () => {
  ...
  it.skip("should successfully use Gemini model via OpenRouter for calendar operations", async () => {
    const userMessage =
    ...
    expect(result.action).toBe("confirm_event_details");

    // Validate the AI correctly parsed the event details
    expect(result.proposed_event).toBeDefined();
    expect(result.proposed_event?.summary).toContain("John");
    expect(result.proposed_event?.summary).toMatch(/meeting|project review/i);

    // Validate the AI set appropriate date/time (tomorrow at 2pm)
    expect(result.proposed_event?.start?.dateTime).toBeDefined();
    if (result.proposed_event?.start?.dateTime) {
      const startTime = new Date(result.proposed_event.start.dateTime);
      expect(startTime.getHours()).toBe(14); // 2pm = 14:00
    }

    // Validate description mentions project review or related terms (if present)
    if (result.proposed_event?.description) {
      expect(result.proposed_event.description).toMatch(
        /project review|project progress|discuss.*project/i
      );
    } else {
      // If no description, at least the summary should indicate it's about project review
      expect(result.proposed_event?.summary).toMatch(
        /project.*review|review.*project/i
      );
    }
  ...
  it.skip("should handle complex calendar queries with Gemini model", async () => {
