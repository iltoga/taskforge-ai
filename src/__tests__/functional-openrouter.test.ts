...
describe('Functional Test: OpenRouter Integration', () => {
  ...
  it('should correctly process a calendar request using Gemini model via OpenRouter', async () => {
    const userMessage = "create a meeting with John tomorrow at 2pm about project review";
    ...
    const result = await aiService.processMessage(userMessage, mockExistingEvents, 'google/gemini-2.0-flash-001');
  ...
  it('should handle translation using OpenRouter model', async () => {
    const italianMessage = "crea un incontro con Marco domani alle 15 per discutere del progetto";

    console.log('Translating Italian message using OpenRouter:', italianMessage);

    // Test translation
    const translatedMessage = await aiService.translateToEnglish(italianMessage);

    console.log('Translated message:', translatedMessage);

    // Should translate to English
    expect(translatedMessage).toContain('Marco');
    expect(translatedMessage.toLowerCase()).toMatch(/meeting|appointment|event/);
  ...
  it('should work with both OpenAI and OpenRouter models', async () => {
    ...
    const openAIResult = await aiService.processMessage(userMessage, mockEvents, 'gpt-4o-mini');
    console.log('OpenAI result:', openAIResult.type);

    // Test with OpenRouter Gemini model
    const openRouterResult = await aiService.processMessage(userMessage, mockEvents, 'google/gemini-2.0-flash-001');
